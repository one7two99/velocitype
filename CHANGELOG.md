...
### Added
- Coach drills now show their **provenance**: the drill banner displays which LLM
  generated the words (e.g. `✨ mistral-small-latest` / `✨ qwen3.5:4b`) or, when
  the model was unavailable or its output unusable, `⚙ offline fallback`. The core
  "generated by your chosen LLM" behaviour is now visible and verifiable.

### Changed
- **Bigram-focused drills fall back far less often.** The coverage check for
  letter-pair drills was too strict under a restricted (progressive-unlock)
  alphabet — it demanded each pair appear twice, which the model often couldn't
  satisfy, forcing the deterministic generator. It now requires that at least half
  the focus pairs appear and that they recur a few times overall. Drill generation
  also gets a bigram-specific prompt hint, one more retry, and a larger token
  budget so more LLM output survives.
...
[Unreleased]: https://github.com/one7two99/velocitype/compare/v0.33.0...HEAD
[0.33.0]: https://github.com/one7two99/velocitype/compare/v0.32.0...v0.33.0
