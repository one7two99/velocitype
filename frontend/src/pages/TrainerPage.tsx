...
import type { CoachDrill, WeakKeyInfo } from "../api/types";
...
export function TrainerPage() {
  ...
  // Provenance of the active coach drill: which LLM produced it, or "fallback".
  const [drillInfo, setDrillInfo] = useState<{ source: string; model: string } | null>(null);
  ...
  const nextDrillRef = useRef<Promise<CoachDrill> | null>(null);
    ...
    async (opts?: { reuseLesson?: string }) => {
      ...
      try {
        ...
        if (opts?.reuseLesson) {
          ...
          let drill: CoachDrill | null = null;
          ...
          if (!drill) drill = await coachApi.drill(layoutId, focusKeys, focusBigrams);
          text = drill.lesson;
          setDrillInfo({ source: drill.source, model: drill.model });
          resp = await sessionsApi.start({
            layout_id: layoutId,
            mode: "coach_drill",
            custom_text: drill.lesson,
          });
  ...
  const handleComplete = useCallback(
    async (r: EngineResult) => {
      ...
      try {
        ...
        if (drillActive) {
          nextDrillRef.current = coachApi.drill(layoutId, focusKeys, focusBigrams);
        }
      } catch (err) {
        setSaveError(
    ...
    <>
      {drillActive && (
        <div className="tf-drill-banner">
          ...
          {drillInfo && (
            <span
              className={`tf-drill-source${drillInfo.source === "fallback" ? " is-fallback" : ""}`}
              title={
                drillInfo.source === "fallback"
                  ? "The LLM was unavailable or its output was unusable — this drill was built by the deterministic generator."
                  : `Generated by ${drillInfo.source} (${drillInfo.model})`
              }
            >
              {drillInfo.source === "fallback"
                ? "⚙ offline fallback"
